...
const ComprehensiveAnalysis = ({ analysis, onClose }) => {
  ...
  const getColorClasses = (type) => {
    const classes = {
      headerBg: color === 'green' ? 'from-green-600 via-green-600 to-green-600' :
                color === 'blue' ? 'from-blue-600 via-blue-600 to-blue-600' :
                color === 'yellow' ? 'from-yellow-600 via-yellow-600 to-yellow-600' :
                'from-red-600 via-red-600 to-red-600',
      headerText: color === 'green' ? 'text-green-100' :
                  color === 'blue' ? 'text-blue-100' :
                  color === 'yellow' ? 'text-yellow-100' :
                  'text-red-100',
      border: color === 'green' ? 'border-l-green-500' :
      ...
      icon: color === 'green' ? 'text-green-600' :
            ...
            'text-red-600',
      bg: color === 'green' ? 'bg-green-50 border-green-200' :
      ...
      text: color === 'green' ? 'text-green-800' :
            ...
            'text-red-800',
      button: color === 'green' ? 'from-green-600 to-green-600 hover:from-green-700 hover:to-green-700' :
              color === 'blue' ? 'from-blue-600 to-blue-600 hover:from-blue-700 hover:to-blue-700' :
              color === 'yellow' ? 'from-yellow-600 to-yellow-600 hover:from-yellow-700 hover:to-yellow-700' :
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className={`bg-gradient-to-r ${getColorClasses('headerBg')} text-white p-6 rounded-t-3xl`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Award className="w-8 h-8" />
              <div>
                <h1 className="text-3xl font-bold">Performance Analysis</h1>
                <p className={getColorClasses('headerText')}>Mentor-Style Feedback for Your Growth</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <Card className={`border-l-4 ${getColorClasses('border')}`}>
            ...
            <CardContent>
              <div className={`${getColorClasses('bg')} p-4 rounded-lg border`}>
                <p className={`${getColorClasses('text')} text-lg font-medium mb-2`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">
                    ({analysis.completedTests}/{analysis.totalTests} tests completed)
                  </span>
                </div>
              </div>
            </CardContent>
                ...
                <div className="space-y-3">
                  {analysis.testBreakdown.map((test, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                      <div className="flex items-center gap-3">
                        {test.test === "Aptitude Test" && <Target className="w-5 h-5 text-blue-600" />}
                        {test.test === "Coding Challenge" && <Code className="w-5 h-5 text-green-600" />}
                        {test.test === "Interview" && <MessageSquare className="w-5 h-5 text-purple-600" />}
          ...
          <Card className={`border-l-4 ${getColorClasses('border')}`}>
            ...
            <CardContent>
              <div className={`${getColorClasses('bg')} p-4 rounded-lg border`}>
                <p className={getColorClasses('text')}>
                  {analysis.aiInsights}
                </p>
              </div>
            </CardContent>
        ...
        <div className="p-6 bg-gray-50 rounded-b-3xl">
          <div className="text-center">
            <button
              onClick={onClose}
              className={`bg-gradient-to-r ${getColorClasses('button')} text-white px-8 py-3 rounded-xl font-bold transition-all transform hover:scale-105 shadow-lg`}
            >
              {analysis.completedTests === 3 ? "Complete Game" : "Continue Improving"}
            </button>
